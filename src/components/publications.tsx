...
export class PublicationSummaryDisplay extends React.Component<PublicationSummaryDisplayProps, {}> {
    ...
    public render(): JSX.Element {
        ...
        if (this.props.detailLevel === PublicationDetailLevel.title) {
            return <Link className="paper-title" to={`/${downloadName}`} itemProp="name">{data.title}</Link>;
        } else {
            return <div className="paper row" itemScope itemType="http://schema.org/ScholarlyArticle" data-authors={authors.map((a) => a.id).join(',')}>
                <div className="col-sm-10">
                    <Link className="paper-title" to={`/${downloadName}`} itemProp="name">{data.title}</Link>
