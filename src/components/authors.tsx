    ...
    public render(): JSX.Element {
        ...
        if (this.props.withLinks) {
            if (data.use_local_homepage) {
                return <span itemProp="author" data-author-id={data.id} className={className}><Link className="author-internal-link" to={`/${data.given_name}_${data.family_name}`}>{`${data.given_name} ${data.family_name}`}</Link></span>
            } else if (data.homepage) {
                return <span itemProp="author" data-author-id={data.id} className={className}><a className="author-external-link" href={data.homepage} target="_blank">{`${data.given_name} ${data.family_name}`}</a></span>
            }
        }
        return <span data-author-id={data.id} className={className} itemProp="author">{`${data.given_name} ${data.family_name}`}</span>
    }
}
